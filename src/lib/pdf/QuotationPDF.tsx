...
const ITEMS_PER_SUBSEQUENT_PAGE = 14;

// แบ่งรายการเป็นหน้าๆ
function splitItemsIntoPages(items: QuotationPDFProps["items"]) {
  const pages: { items: QuotationPDFProps["items"]; startIndex: number; carryForward?: number; carryOver?: number }[] = [];
      ...
      {isFirstPage ? (
        ...
        <View style={styles.header}>
          <View style={styles.companyInfo}>
            ...
            <Text style={styles.companyDetail}>
              เลขประจำตัวผู้เสียภาษี: {company?.tax_id || "-"} ({!company?.branch_code || company.branch_code === "00000" ? "สำนักงานใหญ่" : `สาขา: ${company.branch_name || company.branch_code}`})
            </Text>
            <Text style={styles.companyDetail}>
        ...
        <View style={styles.headerCompact}>
          <View style={styles.headerCompactLeft}>
            ...
            <Text style={styles.headerCompactText}>
              ลูกค้า: {quotation.customer_name}
            </Text>
          </View>
      ...
      {isFirstPage && (
        <View style={styles.customerSection}>
          ...
          {quotation.customer_name_en && (
            <Text style={styles.customerDetail}>{quotation.customer_name_en}</Text>
          )}
...
export function QuotationPDF({ quotation, items, company, showStamp = true, showSignature = true }: QuotationPDFProps) {
