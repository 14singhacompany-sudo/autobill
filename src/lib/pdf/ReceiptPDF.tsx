...
function splitItemsIntoPages(items: ReceiptPDFProps["items"]) {
  const pages: { items: ReceiptPDFProps["items"]; startIndex: number; carryForward?: number; carryOver?: number }[] = [];
          ...
          <View style={styles.documentTitle}>
            ...
            {receipt.sales_channel && getSalesChannelConfig(receipt.sales_channel) && (
              <View style={{ flexDirection: "row", justifyContent: "flex-end", alignItems: "center", marginTop: 4, alignSelf: "flex-end" }}>
                ...
                <Text style={[styles.salesChannelBadge, { backgroundColor: getSalesChannelConfig(receipt.sales_channel)!.color }]}>
                  {getSalesChannelConfig(receipt.sales_channel)!.label}
                </Text>
              </View>
            )}
          </View>
      ...
      {isFirstPage && (
        <View style={styles.customerSection}>
          ...
          {receipt.customer_name_en && (
            <Text style={styles.customerDetail}>{receipt.customer_name_en}</Text>
          )}
          {receipt.customer_address && (
            <Text style={styles.customerDetail}>{receipt.customer_address}</Text>
          )}
          ...
          {receipt.customer_contact && (
            <Text style={styles.customerDetail}>
              ผู้ติดต่อ: {receipt.customer_contact}
            </Text>
          )}
            ...
            <View style={styles.summaryBox}>
              ...
              {receipt.discount_amount > 0 && (
                <View style={styles.summaryRow}>
                  ...
                  <Text style={styles.discountText}>
                    -{formatNumber(receipt.discount_amount)}
                  </Text>
                </View>
              )}
              <View style={styles.summaryRow}>
                <Text>มูลค่าก่อน VAT</Text>
                <Text>{formatNumber(receipt.amount_before_vat)}</Text>
              </View>
              <View style={styles.summaryRow}>
          ...
          <View style={styles.summaryTotalRow}>
            ...
            <View style={styles.summaryTotal}>
              ...
              <Text style={styles.summaryTotalText}>
                {formatNumber(receipt.total_amount)} บาท
              </Text>
            </View>
          </View>
            ...
            <View style={styles.notesSection}>
              {receipt.notes && (
                <View style={{ marginBottom: 8 }}>
                  ...
                  <Text style={styles.customerDetail}>{receipt.notes}</Text>
                </View>
              )}
              {receipt.terms_conditions && (
                <View>
...
export function ReceiptPDF({ receipt, items, company, showStamp = true, showSignature = true }: ReceiptPDFProps) {
