...
interface BillingInvoiceFormProps {
  initialData?: Partial<BillingInvoiceFormData>;
  onSubmit?: (data: BillingInvoiceFormData, action: "save" | "send") => Promise<void>;
  onAutoSave?: (data: BillingInvoiceFormData) => Promise<{ id: string; invoice_number: string } | null>;
...
export function BillingInvoiceForm({
  ...
  const getDefaultDueDate = () => {
    const d = new Date();
    d.setDate(d.getDate() + 30); // Default due date 30 days from now
    return getLocalDateString(d);
  };

  const [formData, setFormData] = useState<BillingInvoiceFormData>({
    customer_name: "",
    customer_name_en: "",
    customer_address: "",
    customer_tax_id: "",
    ...
    items: [],
    vat_rate: 7, // ใบแจ้งหนี้มี VAT 7% เป็นค่าเริ่มต้น
  ...
  useEffect(() => {
    ...
    if (initialData && !initialDataLoadedRef.current) {
      ...
      setFormData({
        ...
        payment_terms: initialData.payment_terms || "ชำระภายใน 30 วัน",
      });
  ...
  const paymentTermsOptions = [
    { value: "ชำระภายใน 7 วัน", label: "ชำระภายใน 7 วัน" },
    { value: "ชำระภายใน 15 วัน", label: "ชำระภายใน 15 วัน" },
    { value: "ชำระภายใน 30 วัน", label: "ชำระภายใน 30 วัน" },
    { value: "ชำระภายใน 45 วัน", label: "ชำระภายใน 45 วัน" },
    { value: "ชำระภายใน 60 วัน", label: "ชำระภายใน 60 วัน" },
    { value: "ชำระทันที", label: "ชำระทันที" },
  ];
  ...
  const updateField = <K extends keyof BillingInvoiceFormData>(
    field: K,
    ...
    if (action === "send") {
      if (!formData.customer_name || formData.customer_name.trim() === "") {
        toast({
          ...
          description: "กรุณาระบุชื่อลูกค้าก่อนออกใบแจ้งหนี้",
          variant: "destructive",
        });
        return;
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <CustomerBranchSelector value={formData.customer_branch_code} onChange={(value) => updateField("customer_branch_code", value)} readOnly={readOnly} name="billing_invoice_customer_branch_type" />
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            {/* เงื่อนไขการชำระเงิน */}
            <div className="space-y-1.5">
              <Label htmlFor="payment_terms" className="text-sm font-medium">เงื่อนไขการชำระเงิน</Label>
              {readOnly ? (
                <Input
      ...
      <div className="flex flex-col gap-4 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          {readOnly ? (
            ...
            <>
              {currentDocumentId && (
                <Link href={`/billing-invoices/${currentDocumentId}/preview`}>
                  <Button>
