...
export function InvoiceForm({
  ...
  const totals = useMemo(() => {
    // 1. คำนวณยอดรวม (ราคาตามที่ตั้งไว้ ยังไม่หักส่วนลด)
    ...
    if (action === "send") {
      if (!formData.customer_name || formData.customer_name.trim() === "") {
        toast({
          ...
          description: "กรุณาระบุชื่อผู้ซื้อก่อนออกใบกำกับภาษี",
          variant: "destructive",
        });
        return;
      }
  ...
  const handlePreview = async () => {
    ...
    try {
      // Read number inputs directly as a final safeguard for mobile browsers
      // that may not flush the last input event before the preview click.
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <CustomerBranchSelector value={formData.customer_branch_code} onChange={(value) => updateField("customer_branch_code", value)} readOnly={readOnly} name="invoice_customer_branch_type" />
            </div>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-4">
          {readOnly ? (
            ...
            <>
              ...
              {/* เอกสารที่ออกแล้วให้ไปหน้าพรีวิวเพื่อพิมพ์หรือดาวน์โหลด */}
              {documentStatus === "draft" || !documentStatus ? (
                ...
                <Button disabled={isSubmitting || !currentDocumentId} onClick={() => router.push(`/invoices/${currentDocumentId}/preview`)}>
                  <Eye className="h-4 w-4 mr-2" />
                  ดูใบกำกับภาษี
                </Button>
              )}
